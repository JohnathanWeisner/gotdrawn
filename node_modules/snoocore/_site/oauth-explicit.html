...
<html>
  ...
  <link rel="canonical" href="http://tsenior.com/snoocore/oauth-explicit.html">
...
<p>Unlike script-based apps, explicit apps can not authenticate using a username &amp; password. The main difference is that you must wait for a user of your application to authenticate with Reddit (e.g. you must wait for reddit to get back with with an authorization code before moving on)</p>

<h2 id="supported-apps">Supported Apps</h2>

<p>Explicit based OAuth will only work if you app is a <code>web</code> or <code>installed</code> application.
b</p>

<h2 id="initial-config">Initial Config</h2>

<p>In your initial config, give it the OAuth settings for your application:</p>
...
<p>This is only needed for web applications. You can leave it an empty string, or leave it out all together for installed applecations.</p>

<h4 id="oauth.mobile"><code>oauth.mobile</code></h4>

<p>Optional. Set mobile to true if you want to send the user to the mobile reddit website for authentication.</p>

<h4 id="oauth.duration"><code>oauth.duration</code></h4>

<p>If you are doing temporary things, there isn&#39;t a need to set this. If you want to do more long term tasks (more than an hour) you will need to set this to <code>&#39;permanent&#39;</code>. Read the section on <a href="https://github.com/reddit/reddit/wiki/OAuth2#authorization">authorization</a> on the Reddit OAuth documentation for more information.</p>
...
<p>For CSRF prevention, you should set a <code>state</code>. This field is optional if you don&#39;t want to perform this check. Read more on this below.</p>

<h2 id="handling-the-response">Handling the response</h2>

<p>After the user visits the URL that <code>reddit.getExplicitAuthUrl</code> generates, they will be presented with the option to Allow or Deny the app. After they allow (or disallow) your application, Reddit will redirect the user back to the given <code>redirectUri</code> (set in the initial config)  with the following url parameters:</p>

<ul>
<li><strong>error</strong> - something went wrong with the request</li>
<li><strong>code</strong> - our <code>authorizationCode</code> (used below)</li>
...
<p>For CSRF prevention, you should check that the <code>state</code> in the url parameters is the same as the <code>state</code> specified when generating the authentication url in <code>reddit.getExplicitAuthUrl</code>.</p>

<h2 id="authenticating-with-the-returned-code">Authenticating with the returned code</h2>

<p>Now that we have our <code>authorizationCode</code>, we need to make one more call to Reddit to get back out authorization data:</p>
...
<p>After this, we are able to use the OAuth API calls that Reddit offers.</p>
...
<p>However, it is not persistant. When your script exits and restarts again, you will have to notify Snoocore of what refresh token to use. When authenticating for the first time with <code>reddit.auth</code> (see previous section) it will provide you a refreshToken. You should save this token somewhere (database, etc.) for re-authentication at a later date.</p>
...
<p>There won&#39;t be a need to use the <code>reddit.auth</code> call to authenticate a user again unless they have revoked access to your application.</p>
...
<p>Generally it is a good idea to call this everytime you are finished using the users data. If you need to use their data again re-authenticate by calling <code>reddit.refresh</code>.</p>
