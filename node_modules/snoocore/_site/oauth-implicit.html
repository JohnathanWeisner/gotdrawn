...
<html>
  ...
  <link rel="canonical" href="http://tsenior.com/snoocore/oauth-implicit.html">
...
<p>See an example <a href="https://github.com/trevorsenior/snoocore-examples/blob/master/node/oauth-implicit.js">here</a>.</p>
...
<p>For CSRF prevention, you should check that the <code>state</code> in the url parameters is the same as the <code>state</code> specified when generating the authentication url in <code>reddit.getImplicitAuthUrl</code>.</p>

<h3 id="authenticating-with-the-returned-code">Authenticating with the returned code</h3>

<p>Now that we have our <code>accessToken</code>, we can now authenticate with reddit and start making calls on behalf of a user.</p>
...
<p>Generally it is a good idea to call this everytime you are finished using the users data.</p>
</div>
    </div>
